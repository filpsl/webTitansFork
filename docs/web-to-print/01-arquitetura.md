...
- Roda o app React/Vite servido pela Vercel.
- Usa apenas a **anon key** do Supabase (`VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY`),
...
- Roda as Serverless Functions em `api/` (runtime Node).
- Detém a **`SUPABASE_SERVICE_ROLE_KEY`** (bypassa RLS) e os segredos do Mercado Pago
  ...
  ambiente **sem** prefixo `VITE_`, então nunca vão para o bundle do cliente.
...
- **Timeout de 10s** nas Serverless Functions — insuficiente para receber e repassar um
  PDF de dezenas de MB.
