...
- [ ] 1.3 (produção) Rodar a `0007` no SQL Editor e confirmar com `\d fila_impressao` que a coluna existe com default 1 e check `>= 1`.
...
- [x] 6.2 Alterar `enviar_para_impressora` para receber a quantidade e usar `lp -d <printer> -n <quantidade_copias>` em vez de `-n 1`.
...
- [ ] 7.2 Testar no site: definir 2 cópias num PDF de N páginas, conferir total exibido = `N * 2 * preço` e que o valor do PIX bate com o exibido.
- [ ] 7.3 Conferir na `fila_impressao` que `quantidade_copias` foi gravado e que `valor_centavos` (autoritativo do servidor) reflete o multiplicador.
- [ ] 7.4 Atualizar o `print-worker` na máquina da sede e validar com um pedido de 2+ cópias que a impressora produz a quantidade correta.
