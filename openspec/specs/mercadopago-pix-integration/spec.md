...
O sistema SHALL expor uma Vercel Serverless Function em `api/payments/create-pix.ts` (runtime Node) que recebe `{ pedidoId: string }` no body JSON e devolve `{ qr_code_base64, qr_code_copia_cola, expiration_date_to, mp_payment_id }`.
